...
export default function ChatInterface({
  ...
  const generateSummary = async () => {
    ...
    try {
      const apiUrl = import.meta.env.VITE_API_URL || "http://localhost:8000";
      const response = await fetch(
        `${apiUrl}/api/conversations/${conversation.id}/summary`,
        {
          method: "POST",
        },
      );
  ...
  const exportSummaryAsPDF = () => {
    ...
    doc.text(
      `Languages: ${conversation.doctor_lang} ↔ ${conversation.patient_lang}`,
      margin,
      margin + 20,
    );
    ...
    doc.text("Generated by MediTranslate", pageWidth / 2, footerY, {
      align: "center",
    });

    doc.save(
      `${conversation.patient_name}_summary_${new Date().toISOString().split("T")[0]}.pdf`,
    );
  };
  ...
  const sourceLang =
    currentRole === "doctor"
      ? conversation?.doctor_lang
      : conversation?.patient_lang;
  const targetLang =
    currentRole === "doctor"
      ? conversation?.patient_lang
      : conversation?.doctor_lang;
  ...
  return (
    <div className="flex-1 flex flex-col bg-white h-screen max-h-screen">
      ...
      <Tabs defaultValue="chat" className="flex-1 flex flex-col min-h-0">
        ...
        <TabsContent
          value="chat"
          className="flex-1 flex flex-col min-h-0 mt-2 mx-2 lg:mx-4 data-[state=inactive]:hidden"
        >
          {/* Messages - Scrollable */}
        ...
        <TabsContent
          value="summary"
          className="flex-1 overflow-y-auto p-2 lg:p-4 data-[state=inactive]:hidden"
        >
          <div className="max-w-2xl mx-auto">
            ...
            {!summary && !loadingSummary && (
              <div className="text-center text-gray-500 mt-8 text-sm">
                Click "Generate Summary" to create an AI-powered medical
                summary.
              </div>
            )}
          </div>
